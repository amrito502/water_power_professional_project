    ...
    <div class="row">
        <div class="col-md-12 col-12 col-lg-12">
            <div class="card px-3 py-4">
                <h2 style="font-weight: 700;color: #383535;margin-bottom: 15px;">Edit Designation</h2>
                <div class="url_page">
                    <p style="font-size: 15px;font-weight: 400;margin-bottom: 0px;">
                        <a href="{{ url('/') }}">Home</a> / Settings / HR Module / Edit Designation
                    </p>
            ...
            <div class="card p-5">
                <form action="{{ route('designations.update', $designation->id) }}" method="POST" class="form" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" name="name" id="name" placeholder="Name" class="form-control py-2 mt-3" value="{{ old('name', $designation->name) }}">
                    </div>


                    <div class="form-group mb-3">
                        <label for="department_id" style="font-size: 16px;font-weight: 500;">Select Department</label>
                        <select name="department_id" id="department_id" class="form-control py-2 mt-3">
                            <option value="">-- Select Department --</option>
                            @foreach ($departments as $department)
                                <option value="{{ $department->id }}"
                                    {{ $department->id == $designation->department_id ? 'selected' : '' }}>
                                    {{ $department->name }}
                                </option>
                            @endforeach
                        </select>
